...
function Profile({setData}) {
    ...
    const userSchema = Yup.object().shape({
        ...
        surname:Yup.string().min(1,'Too short').max(15, 'Too long').required('Reuired')
    })
    ...
    return (
        ...
        {image && !croppedImage && (
            <button type='submit' variant='contained' onClick={cropImage} className='reg-btn'>
            Crop
            </button>
        )}
        {croppedImage && <img src={croppedImage} alt='img' className='center'/>}
        {croppedImage && (
